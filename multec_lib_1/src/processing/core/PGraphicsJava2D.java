...
//        System.out.println("not doing anything special in endDraw()");
      ...
      //loadPixels();  // It seems that nothing is actually done in this method
    }
...
//          op.filter(image, image);
  ...
  public PShape createShape(PShape source) {
    return PShape.createShape(parent, source);
  }
  ...
  static protected PShape createShapeImpl(PApplet parent, int type) {
    PShape shape = null;
    if (type == PConstants.GROUP) {
      shape = new PShape(PConstants.GROUP);
    } else if (type == PShape.PATH) {
      shape = new PShape(PShape.PATH);
    } else if (type == PShape.GEOMETRY) {
      shape = new PShape(PShape.GEOMETRY);
    }
    shape.is3D(false);
    return shape;
  }
  ...
  static protected PShape createShapeImpl(PApplet parent,
                                          int kind, float... p) {
    PShape shape = null;
    int len = p.length;
    ...
    if (kind == POINT) {
      ...
      shape = new PShape(PShape.PRIMITIVE);
      shape.setKind(POINT);
    } else if (kind == LINE) {
      if (len != 4) {
        showWarning("Wrong number of parameters");
...
//      g2.fillRect(0, 0, width, height);
  ...
  // It seems that nothing is actually done in this method.
  @Override
