...
//                      fill, stroke, curveDetail, vertexCode(), kind);
  ...
  public void disableStyle() {
    if (!style) return; // avoid unnecessary work 
    if (openShape) {
  ...
  public void enableStyle() {
    if (style) return; // avoid unnecessary work and a problem when enableStyle is called before disableStyle is called
    if (savedStroke) {
      setStroke(true);
  ...
  protected void usePGraphicsStyles(PGraphics g) {
